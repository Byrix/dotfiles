...
-- NOTE: Me change
-- Move between splits with j and k keys 
-- Resize splits with h and l 
-- Maps the same as my Awesome binds 
...
wk.add({
    ...
    { "s?", ":setlocal spell!<cr>", desc = "toggle [z]pellcheck" },
    { "sl", ":Telescope spell_suggest<cr>", desc = "[l]ist spelling suggestions" },
    { 'sg' }
}, { mode = 'n', silent = true })
...
wk.add({
  {
    ...
    { "<leader>og", ":Neogen<cr>", desc = "neo[g]en docstring" },
    ...
    { '<leader>sg', 'zg', desc='[g]ood' },
    { '<leader>sb', 'zw', desc='[b]ad' },
    { '<leader>ss', '<cmd>Telescope spell_suggest<cr>', desc='[s]uggest' },
    { '<leader>se', '<cmd>setlocal spell spelllang=en_au<cr>', desc='[e]nable' },
    { '<leader>sd', '<cmd>setlocal spell<<cr>', desc='[d]isable' }
  }
}, { mode = 'n'})

-- Custom projections.nvim commands and keymaps
-- NOTE: self-added, not apart of kickstart
...
vim.api.nvim_create_user_command('ProjectSessionStore', function()
  -- Save the current session (assuming cwd is a project dir)
  Session.store(vim.loop.cwd())
end, {})
vim.api.nvim_create_user_command('ProjectSessionRestore', function()
  -- Restore a session (assuming cwd is a project dir)
  Session.restore(vim.loop.cwd())
end, {})

-- Add above commands to leader group 
wk.add({
  { "<leader>p", group = "[p]rojects" },
  { "<leader>ps", "<cmd>ProjectSessionStore<cr>", desc="[s]ave session"},
  { "<leader>pr", "<cmd>ProjectSessionRestore<cr>", desc="[r]estore session"},
  { "<leader>pa", "<cmd>AddWorkspace<cr>", desc="[a]dd workspace"},
  { "<leader>pf", "<cmd>Telescope projections<cr>", desc='[f]ind project'},
}, { mode = 'n'})

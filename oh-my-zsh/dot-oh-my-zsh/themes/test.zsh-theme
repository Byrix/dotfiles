...
$fg[blue]%D{%H%M}%{$reset_color%} %{%(!.${fg[red]}.${fg[white]})%}$%{$reset_color%} '

PROMPT2='%fg[blue]%D{%L:%M}%{$reset_color%} %{%(!.${fg[red]}.${fg[white]})%}$%{$reset_color%} '
...
if [[ -n $CONDA_DEFAULT_ENV ]]; then
  RPROMPT="%fg[green]($CONDA_DEFAULT_ENV)%f ${RPROMPT}"
fi
